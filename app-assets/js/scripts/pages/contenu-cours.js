          ...
          .then(function (docFormation) {
            ...
            firestore.collection('users').doc(docUser.data().idAdmin).collection('classes').doc($('select#selectClass2').val()).update({
              ...
              firestore.collection('users').doc(docUser.data().idAdmin).collection('classes').doc($('select#selectClass2').val()).collection(themeValue).doc('test').set({
                ...
                console.log("done");
                swal({
                  title: "Le thème a bien été ajouté.",
                  type: "success",
                  html: true,
                  closeOnConfirm: true
                }, function (isConfirm) {
    ...
    firestore.collection('users').doc(user.uid).get()
      .then(function (docUser) {
        firestore.collection('users').doc(docUser.data().idAdmin).collection('classes').doc(formationValue).set({
          ...
          swal({
            title: "La formation a bien été ajoutée.",
            type: "success",
            html: true,
            closeOnConfirm: true
          }, function (isConfirm) {
      ...
      firestore.collection('users').doc(docUser.data().idAdmin).collection('classes').doc($('select#selectClass2').val()).get()
        .then(function (doc) {
          if (doc.data().matieres.length > 0) {
            ...
            document.getElementById('existingThemeList').innerHTML = '<p>Aucun thème n\'a encore été ajouté à la formation sélectionnée';
          }
  ...
  if (soutien == true) {
    ...
    switch (userCategory) {
      ...
      case 'admin':
        ...
        var nav4bis = '<li data-menu="dropdown" class="dropdown nav-item "><a href="#" data-toggle="dropdown" class="dropdown-toggle nav-link"><i class="fas fa-pencil-ruler"></i><span data-i18n="nav.dash.main">Outils pédagogiques</span></a><ul class="dropdown-menu"><li data-menu=""><a href="../../bibliotheque/emsatconnect/pages/mes-outils.php"  class="dropdown-item"><span data-i18n="nav.dash.main">Créer un contenu - Formation</span></a></li><li data-menu=""><a href="../../bibliotheque/emsatconnect/pages/mes-outils-scratch.php"  class="dropdown-item"><span data-i18n="nav.dash.main">Créer un contenu - Bibliothèque</span></a></li></ul></li>';
...
function initChapters() {
  ...
  var sponshtml6 = "</div></p><p> </p><h6 class=' card-title text-bold-600'>Indicateurs de réussite</h4><p class='card-text text-xs-left'><div style='border-width: thin; border-color:black; border-style:solid'>";
  ...
  var sponshtml15 = "\");'>Créer un contenu</button></th>";
...
function createFormationAndModule() {
  ...
  if (!noInput) {
    ...
    alert("Attention ! Vous devez créer au moins 1 module.");
  }
}
...
function updateFormation() {
  ...
  firestore.collection('users').doc(idAdmin).collection('classes').doc(document.getElementById('formationModifyTitle').innerText).get()
    .then(function (doc) {
      ...
      if (noInput) {
        alert('Attention, vous devez avoir au moins 1 module dans votre formation. Fermez et réouvrez la fenêtre de modification.');
      }
      else {
