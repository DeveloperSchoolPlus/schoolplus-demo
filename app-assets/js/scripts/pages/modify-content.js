...
$('select#selectClass').on('change', function () {
  var user = auth.currentUser;
  document.getElementById('selectContent').selectize.clearOptions();
...
$('select#selectContent').on('change', function() {
  var user = auth.currentUser;
  // document.getElementById('selectContent').selectize.clearOptions();
  firestore.collection('users').doc(user.uid).get()
  .then(function(docUser) {
    ...
    .then(function(docChapter) {
      ...
      docChapter.data().createdContent.forEach(function(elem) {
        if(elem.contentId == $('select#selectContent').val()) {

          document.getElementById('contentTitle').value = elem.title;
          document.getElementById('oldContentTitle').value = elem.title;
          switch(elem.contentType) {
            case 'cours':
              document.getElementById('radioCours').checked = true;
              document.getElementById('radioExercices').checked = false;
              document.getElementById('radioEvaluationF').checked = false;
              document.getElementById('radioEvaluationC').checked = false;

              break;
            case 'exercices':
              document.getElementById('radioCours').checked = false;
              document.getElementById('radioExercices').checked = true;
              document.getElementById('radioEvaluationF').checked = false;
              document.getElementById('radioEvaluationC').checked = false;
              break;
            case 'evaluationF':
              document.getElementById('radioCours').checked = false;
              document.getElementById('radioExercices').checked = false;
              document.getElementById('radioEvaluationF').checked = true;
              document.getElementById('radioEvaluationC').checked = false;
              break;
              case 'evaluationC':
              document.getElementById('radioCours').checked = false;
              document.getElementById('radioExercices').checked = false;
              document.getElementById('radioEvaluationF').checked = false;
              document.getElementById('radioEvaluationC').checked = true;
              break;
            default:
              document.getElementById('radioCours').checked = false;
              document.getElementById('radioExercices').checked = false;
          ...
          scope.$apply(function () {
            scope.htmlcontent = elem.content;
...
function initContentSelection() {
  ...
  .then(function(docUser) {
    ...
    firestore.collection('users').doc(docUser.data().idAdmin).collection('classes').doc($('select#selectClass').val()).collection($('select#selectMatiere').val()).doc($('select#selectChapter').val()).get()
    .then(function(docChapter) {
      if(docChapter.data().createdContent == undefined || docChapter.data().createdContent.length == 0) {
        alert('A ce jour, aucun contenu n\'a été créé pour ce chapitre. Rendez-vous dans la section "Créer un contenu de cours" pour en créer un.' );
      }
      else
      {
        docChapter.data().createdContent.forEach(function(elem) {
          ...
          {
            contentType = 'Fiche de cours';
          } 
          else if(elem.contentType == 'exercices')
          {
            contentType = "Fiche d'exercices"
          }
          else if(elem.contentType == 'evaluationF')
          {
            contentType = 'Evaluation formative'
          }
          else if(elem.contentType == 'evaluationC')
          {
            contentType = 'Evaluation certificative'
          }

          document.getElementById('selectContent').selectize.addOption({ value: elem.contentId, text: elem.title + ' - '+ contentType});
          var $select = $('select#selectContent').selectize();
          var control = $select[0].selectize;
          control.clear();
        });
      }
...
$('#btnDeleteContent').on('click', function() {
  ...
  swal({
    title: "Attention !",
    text: "Souhaitez-vous vraiment supprimer ce contenu ? Cette opération est irréversible.",
    type: "warning",
    html: true,
    ...
    if(isConfirm) {
      if($('select#selectClass').val() != '' && $('select#selectMatiere').val() != '' && $('select#selectChapter').val() != '' && $('select#selectContent').val() != '') {
        if(document.getElementById('contentTitle') != '') {

          console.log(document.getElementById('oldContentTitle').value);
        ...
        {
          alert("Veuillez donner un titre au contenu à modifier.");
        }
      }
      else
      {
        alert("Veuillez sélectionner un contenu pour le modifier.");
      }
    }
    else
    {
      ...
      .then(function(docUser) {
        ...
        .then(function(docChapter) {
          ...
          existingArray.forEach(function(elem) {
            ...
            if(elem.contentId == $('select#selectContent').val()){
              //Inside the good element
              elem.title = document.getElementById('contentTitle').value;
              elem.content = document.getElementById('newContent').value;
              elem.contentType = $('input[name="radio"]:checked').val();
              ...
              elem.lastModificationDate = dateString;
              elem.lastModificationUserId = user.uid;
              elem.lastModificationUserName = docUser.data().firstName + ' ' + docUser.data().lastName;
            }
