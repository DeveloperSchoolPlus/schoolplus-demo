...
const addExpectedSkills = document.getElementById('addExpectedSkills');
const addChallenges = document.getElementById('addChallenges');
...
function setUserInterface(soutien) {
  ...
  if (soutien == undefined) {
    ...
    navMenu.innerHTML += '<li data-menu="dropdown" class="dropdown nav-item active"><a href="mes-cours.php" class="nav-link"><i class="fas fa-book"></i><span data-i18n="nav.dash.main">Mes cours</span></a></li>';
                  ...
                  if (typeof doc2.data().cours[0] == "object") {
                    ...
                    doc2.data().cours.forEach(function (elem) {
                      ...
                      elem.suivi.forEach(function (elem2) {
                        ...
                        coursStudentIds.push(elem2.studentId);
                      });
                      ...
                      if (!docUser.data().dev) {
                        coursStudentIds.forEach(function (elem) {
                          if (docUser.data().testAccounts.includes(elem)) {

                            coursStudentNames.splice(coursStudentIds.indexOf(elem), 1);
                            coursStudentSmileys.splice(coursStudentIds.indexOf(elem), 1);
                          }
                        });

                        console.log(coursStudentNames);
                      }
                  ...
                  if (typeof doc2.data().exercices[0] == "object") {
                    ...
                    doc2.data().exercices.forEach(function (elem) {
                      elem.suivi.forEach(function (elem2) {
                        ...
                        exoStudentIds.push(elem2.studentId);
                      });
                      ...
                      if(!docUser.data().dev)
                      {
                        exoStudentIds.forEach(function(elem) {
                          if(docUser.data().testAccounts.includes(elem)) {
          
                            exoStudentNames.splice(exoStudentIds.indexOf(elem), 1);
                            exoStudentSmileys.splice(exoStudentIds.indexOf(elem), 1);
                          }
                        });
          
                        console.log(exoStudentNames);
                      }
