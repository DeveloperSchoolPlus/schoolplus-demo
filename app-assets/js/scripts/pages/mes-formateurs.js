        ...
        firestore.collection('users').where('userCategory', '==', 'teacher').where('instituteName', '==', docUser.data().instituteName).get()
          .then(function (querySnapshot) {
            ...
            querySnapshot.forEach(function (docTeachers) {
              ...
              const table7 = '<td style="padding-left: 0.1%; padding-right: 0.1%;"><button class="btn color-blue text-bold-600" name="mail_' + docTeachers.id + '" onclick="sendMail(this.name)"><i class="fas fa-envelope"></i> Contacter</button> </td></tr>'

              /* END USER TABLE */

              var totalDetailHTML = table1 + table2 + table3 + table4 + table5 + table6 + table7;
