      ...
      .then(function(querySnapshot) {
        ...
        querySnapshot.forEach(function(docTeachers) {
          ...
          // {
            ...
            var totalHTML = listFollowingHtml1 +docTeachers.id+ listFollowingHtml2 +docTeachers.id + listFollowingHtml3 + docTeachers.data().firstName + " " + docTeachers.data().lastName +listFollowngHtml+'Apprenant'+ listFollowingHtml4 +docTeachers.id+ listFollowingHtml5 + listFollowingHtml6
    ...
    else if(docUser.data().userCategory == 'admin' || docUser.data().userCategory == 'author'|| docUser.data().userCategory == 'teacher')
    {
      ...
      .then(function(querySnapshot) {
        ...
        querySnapshot.forEach(function(docTeachers) {
          ...
          if(docTeachers.data().matieres != undefined)
          {
            if(docTeachers.data().matieres.length >0)
            {
              if(docTeachers.data().matieres[0] != '')
              {
                formationCount = docTeachers.data().matieres.length;
              }
            }
          }
          ...
          const table7 = '<td style="padding-left: 0.1%; padding-right: 0.1%;"><a href="#!" onClick="showFormation(\''+docTeachers.id+'\')" >'+ formationCount+ string + '</a></td>';


          const table8 = '<td style="padding-left: 0.1%; padding-right: 0.1%;"><button class="btn color-blue text-bold-600 mr-1 ml-1" name="mail_'+docTeachers.id+'" onclick="sendMail(this.name)"><i class="fas fa-envelope"></i> Contacter</button> </td></tr>'
...
function showFormation(idUser) {
  document.getElementById('listFormation').innerHTML = '';
  firestore.collection('users').doc(idUser).get()
  .then(function(docUser) {

    document.getElementById('formationTitle').innerText = ' Formations de '+docUser.data().firstName+' '+docUser.data().lastName;

    if(docUser.data().matieres != undefined) 
    {
      if(docUser.data().matieres.length > 0)
      {
        if(docUser.data().matieres[0] != '')
        {
          document.getElementById('listFormation').style.display ='block';
          document.getElementById('noFormation').style.display = 'none';
          var index = 1;
          docUser.data().matieres.forEach(function(elem) {
            document.getElementById('listFormation').innerHTML += '<h5>'+ index+'. ' + elem.matiere+'</h5>';
            index++;
          });
        }
        else
        {
          document.getElementById('listFormation').style.display ='none';
          document.getElementById('noFormation').style.display ='block';
        }
      }
      else
      {
...
function displayUserInfo(userId) {
  ...
  .then(function(docUser) {
    ...
    switch(docUser.data().userCategory) {
      case 'student':
      modalUserCategory.innerText = 'Apprenant';
      break;
      case 'teacher':
