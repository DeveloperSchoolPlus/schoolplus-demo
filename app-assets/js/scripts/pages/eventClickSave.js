  ...
  query.get().then(function (doc) {
    if (doc.data().editable != true) {
      ...
      $('#formModifyEvent').submit(function (eve) {
        ...
        var newRecur = new Date(newStartDate);
        ...
        if (oldTitle != $('select#selectModifyMatiere').val() || oldStartDate != newStartDate || oldClass != $('select#selectModifyClass').val()) {
          ...
          //First we check if the event is recurrent or not. Depending on the case data to be written will change
          if ($('select#selectModifyRecur').val() == "non") {
            
            //Event is not recurrent, we can start creating new activity for admin, then delete the older one
